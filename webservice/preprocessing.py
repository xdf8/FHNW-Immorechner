...
class preprocessor:
    def __init__(self, df, y_var, cols_to_drop=[], numbers_to_encode=[], method_to_encode='onehot_encode',
                 numbers_to_onehot_encode=[], test_frac=0.2, remove_skew_by_boxcox=False,
                 features_not_to_box_cox=[], standardise_number=True, square_numbers=False, robust_scaler=False,
                 verbose=0):
        ### Tests
        assert not numbers_to_onehot_encode, 'Please change "numbers_to_onehot_encode" to "numbers_to_encode", thx.'
        assert method_to_encode in ['onehot_encode',
                                    'label_encode'], 'For method_to_encode, only "onehot_encode" or "label_encode" is accepted.'
        ...
        ### Drop some rows based on common sense:
        try:
            df = df[(df['FloorNumber'] <= 42) | (df['FloorNumber'].isna())].copy()
        except:
            pass
        try:
    ...
    def __fit_df(self):
        ...
        if self.remove_skew_by_boxcox:
            if self.features_not_to_box_cox:
                skew_features = self.X.select_dtypes(['float32', 'float64']).drop(
                    columns=self.features_not_to_box_cox).apply(lambda x: skew(x)).sort_values(ascending=False)
            else:
                skew_features = self.X.select_dtypes(['float32', 'float64'])
    ...
    def __encode_transform_df(self, _df):
        ...
        for col in self.obj_features:
            if self.label_method == 'onehot_encode':
                ...
                df_onehot_encoded_df = pd.DataFrame(df_onehot_encoded,
                                                    columns=self.d_labler[col].get_feature_names([col]))
        ...
        for col in self.num_features:
            if not df[col].between(0, 1).all():
                if self.robust_scaler:
                    df[col] = ((df[col] - self.median[col]) / (
                            self.third_quantile[col] - self.first_quantile[col])).copy()
                else:
